...
try:
    # Package-relative import for vendored module usage.
    from .font_management import load_fonts
except ImportError:
    # Fallback for direct script execution compatibility.
    from font_management import load_fonts
...
def _normalize_cache_point(point, dist):
    lat, lon = point
    lat_key = round(float(lat), 3)
    lon_key = round(float(lon), 3)
    dist_key = int(round(float(dist) / 250.0) * 250)
    return lat_key, lon_key, dist_key
...
def fetch_graph(point, dist, network_type="all") -> MultiDiGraph | None:
    ...
    """
    lat_key, lon_key, dist_key = _normalize_cache_point(point, dist)
    graph = f"graph_{lat_key}_{lon_key}_{dist_key}_{network_type}"
...
def fetch_features(point, dist, tags, name) -> GeoDataFrame | None:
    ...
    features = f"{name}_{lat_key}_{lon_key}_{dist_key}_{tag_str}"
...
def create_poster(
    ...
    fetch_steps = 1 + int(include_water) + int(include_parks)
    with tqdm(
        total=fetch_steps,
    ...
    if fmt == "png":
        save_kwargs["dpi"] = int(raster_dpi) if raster_dpi else 300
